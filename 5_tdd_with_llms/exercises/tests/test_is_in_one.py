import unittest
from is_in_one import * # type: ignore
...
class TestIsInOne(unittest.TestCase):
  def test_is_in_both(self):
    ...
    self.assertEqual(is_in_one("a", ["a", "b"], ["a", "c"]), False)
    
  def test_is_not_in_both(self):
    self.assertEqual(is_in_one("z", ["a", "b"], ["a", "c"]), False)
    
  def test_is_not_in_one(self):
    self.assertEqual(is_in_one("b", ["a", "b"], ["a", "c"]), True)
